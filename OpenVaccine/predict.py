...
def get_args():
    ...
    parser.add_argument('--path', type=str,
                        default=r'D:\DAQIU\DLGP\SecretJob\190.RNAdegformer-Webapp-main\stanford-covid-vaccine',
                        required=False, help='path of csv file with DNA sequences and labels')
...
for fold in folds:
    ...
    for i in range(1):#5
        ...
        model.load_state_dict(torch.load(f"{opts.weights_path}/fold{fold}top{i+1}.ckpt",map_location=torch.device('cpu')))
...
print("len(preds_to_csv):",len(preds_to_csv))
to_csv=[]
